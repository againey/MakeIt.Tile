...
namespace Experilous.Topological
{
	public static class MeshGenerator
	{
		...
		public static Mesh[] GenerateSubmeshes(IFacesIndexer faces, Vector3[] vertexPositions, Vector3[] repetitionAxes, EdgeWrapData[] edgeWrapData, Vector3[] centroidPositions, Vector3[] faceNormals, System.Func<int, Color> faceColorGenerator)
		{
			List<Mesh> meshes = new List<Mesh>();
			...
			while (i < faceCount)
			{
				...
				while (i < endFaceIndex)
				{
					...
					for (int j = 0; j < neighborCount; ++j, edge = edge.next)
					{
						vertices[meshVertex + j + 1] = PlanarManifold.GetFaceVertexPosition(edge, vertexPositions, edgeWrapData, repetitionAxes);
						colors[meshVertex + j + 1] = new Color(0, 0, 0);
						normals[meshVertex + j + 1] = faceNormals[face];
						triangles[meshTriangle + j * 3 + 0] = meshVertex;
						triangles[meshTriangle + j * 3 + 1] = meshVertex + 1 + j;
